...
class Upcoming_Events extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description'	=>	__( 'Виджет отображает список предстоящих событий', 'uep' )
		);

		parent::__construct(
			'uep_upcoming_events',			//base id
			__( 'Предстоящие события', 'uep' ),	//title
			$widget_ops
		);
	}
	...
	public function form( $instance ) {
		$widget_defaults = array(
			'title'			=>	__('Предстоящие события','wp_panda'),
			'number_events'	=>	5
		);
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Заголовок', 'uep' ); ?></label>
			<input type="text" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" class="widefat" value="<?php echo esc_attr( $instance['title'] ); ?>">
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'number_events' ); ?>"><?php _e( 'Сколько событий показывать', 'uep' ); ?></label>
