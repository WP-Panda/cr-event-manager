...
class iCalendarFile {
	...
	public function create_ics_data_too_file() {
		/** @var string $start Formatted start date and time. Converted to Zulu time. */
		$start = date( 'Ymd', $this->event_start + $this->utc_offset ) . 'T' . date( 'His', $this->event_start + $this->utc_offset ) . 'Z';
		/** @var string $end Formatted end date and time. Converted to Zulu time. */
		$end = date( 'Ymd', $this->event_end + $this->utc_offset ) . 'T' . date( 'His', $this->event_end + $this->utc_offset ) . 'Z';
		...
		$out .= "BEGIN:VCALENDAR\n";
		$out .= "VERSION:2.0\n";
		$out .= "PRODID:-//{$this->organizer}//NONSGML {$this->event_name}//EN\n";
		$out .= "METHOD:REQUEST\n";
		$out .= "BEGIN:VEVENT\n";
		$out .= "UID:" . date( 'Ymd' ) . 'T' . date( 'His' ) . "-" . rand() . "-{$this->organizer}\n"; // Required by Outlook.
		$out .= "DTSTAMP:" . date( 'Ymd' ) . 'T' . date( 'His' ) . "\n";
		$out .= "ORGANIZER:CN={$this->organizer}:MAILTO:{$this->organizer_email}\n";
		$out .= "DTSTART:{$start}\n";
		$out .= "DTEND:{$end}\n";
		$out .= "LOCATION:{$location}\n";
		$out .= "SUMMARY:{$this->event_name}\n";
		$out .= "DESCRIPTION: {$this->event_description}\n";
		$out .= "END:VEVENT\n";
		$out .= "END:VCALENDAR\n";
	...
	public function create_ics_data_too_file_event() {
		/** @var string $start Formatted start date and time. Converted to Zulu time. */
		$start = date( 'Ymd', $this->event_start + $this->utc_offset ) . 'T' . date( 'His', $this->event_start + $this->utc_offset ) . 'Z';
		/** @var string $end Formatted end date and time. Converted to Zulu time. */
		$end = date( 'Ymd', $this->event_end + $this->utc_offset ) . 'T' . date( 'His', $this->event_end + $this->utc_offset ) . 'Z';
